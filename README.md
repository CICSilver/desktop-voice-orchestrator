...
唤醒采用两级检测：KWS 之外，未激活时没有 KWS 命中的语音段会交给整句识别模型查找唤醒词（含首尾一字误听），
命中后走与 KWS 相同的候选、识别和严格解析流程。在 52 条标注录音上，需要唤醒的录音全部唤醒，命令正确
45/52，负样本零误唤醒、零误执行（详见 [`docs/EVALUATION.md`](docs/EVALUATION.md)）。
...
回放模式会为精确最终识别发布解析诊断，也可在调试台手动输入识别文本和可选唤醒词，
查看规范化文本、动作顺序、拒绝错误码与字节偏移；手动解析不会向动作执行器提交计划。
...
- 触发后执行：常驻链路运行音频前端、VAD 和 KWS；开启二级唤醒（`kws.asr_probe`）时，未激活状态下不长于 8 秒的语音段也会在内存中做一次整句识别，只用于查找唤醒词，识别文本在实时模式下既不发布也不写盘
