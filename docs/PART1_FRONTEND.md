...
录制停止事件会在浏览器侧固定当前 20 秒遥测快照，避免后续实时遥测推动已录制波形离开视窗。
冻结视图支持 2/5/10/20 秒缩放、时间滑块、三条轨道直接拖动以及带绝对采样时间的鼠标检查游标；
这些交互仅操作浏览器快照，不向音频处理线程施加背压。
`replay.open` 会停止 WASAPI 并清空音频队列，浏览器也会清空时间轴后只接收 `source=replay` 的遥测。
`live.resume` 会销毁回放实例、重置队列与有状态组件、恢复双路 WASAPI，并通过 `runtime_mode` 通知
浏览器清空回放数据后只接收 `source=live` 的当前音频。
